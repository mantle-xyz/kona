...
impl Encodable for BlobInfo {
    fn rlp_append(&self, s: &mut RlpStream) {
        ...
        match &self.blob_verification_proof {
            Some(blob_verification_proof) => {
                s.append(blob_verification_proof);
            }
            None => {
                s.append_empty_data();
            }
...
impl Decodable for BlobInfo {
    fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
        ...
        Ok(BlobInfo { blob_header, blob_verification_proof })
    }
}
...
impl Decodable for BlobVerificationProof {
    fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
        ...
        let batch_metadata = if !rlp.at(2)?.is_empty() { Some(rlp.val_at(2)?) } else { None };
...
impl Decodable for BatchMetadata {
    fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
        ...
        let batch_header = if !rlp.at(0)?.is_empty() { Some(rlp.at(0)?.as_val()?) } else { None };
...
impl Decodable for BlobHeader {
    fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
        ...
        let commitment = if !rlp.at(0)?.is_empty() { Some(rlp.at(0)?.as_val()?) } else { None };
