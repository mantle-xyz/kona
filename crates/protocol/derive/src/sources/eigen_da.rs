use crate::errors::{BlobProviderError, EigenDAProviderError, PipelineError};
use crate::prelude::ChainProvider;
use crate::proto::{CalldataFrame, calldata_frame};
...
use prost::Message;
use rlp::{Decodable, DecoderError, decode};
use tracing::warn;
    ...
    E: EigenDAProvider + Send,
{
    ...
    async fn load_blobs(
        ...
        debug!(target: "eigen_da", "loading eigen blobs blob hashes len {}, blob data len {}", blob_hashes.len(), blob_data.len());
        if !blob_hashes.is_empty() {
            ...
            debug!("whole blob data size {}", whole_blob_data.len());

            let rlp_blob: VecOfBytes = decode(&whole_blob_data)
                .map_err(|e| EigenDAProviderError::RLPDecodeError(e.to_string()))?;

            debug!("rlp blob data len {}", rlp_blob.0.len());

            for blob in rlp_blob.0 {
                debug!("rlp decode blob vec size {}", blob.len());
                blob_data.push(Bytes::from(blob));
            }
        }
        self.open = true;
        debug!(target: "eigen_da", "loaded eigen blobs blob data len {}", blob_data.len());
        self.data = blob_data;
        Ok(())
    }
...
pub(crate) mod tests {
    ...
    async fn test_calldata_frame_decode() {
        ...
        let mut eigen_da_provider = TestEigenDaProvider::new();
        for tx in txs {
            let (_tx_kind, calldata, _blob_hashes) = match &tx {
                TxEnvelope::Legacy(tx) => (tx.tx().to(), tx.tx().input.clone(), None),
                TxEnvelope::Eip2930(tx) => (tx.tx().to(), tx.tx().input.clone(), None),
                TxEnvelope::Eip1559(tx) => (tx.tx().to(), tx.tx().input.clone(), None),
                TxEnvelope::Eip4844(blob_tx_wrapper) => match blob_tx_wrapper.tx() {
            ...
            if let Some(value) = calldata_frame.value {
                match value {
                    calldata_frame::Value::Frame(_frame) => {}
                    calldata_frame::Value::FrameRef(frame_ref) => {
                        if frame_ref.quorum_ids.is_empty() {
                            warn!(target: "eigen-da-source", "decoded frame ref contains no quorum IDs");
                            continue;
