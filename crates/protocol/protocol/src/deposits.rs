...
const DEPOSIT_EVENT_VERSION_1: B256 =
...
pub enum DepositError {
    ...
    GasDecode(Bytes),
    /// Failed to decode the deposit eth value.
    #[error("Failed to decode the u128 deposit eth value: {0}")]
    EthValueDecode(Bytes),
    /// Failed to decode the deposit eth tx value.
    #[error("Failed to decode the u128 deposit eth tx value: {0}")]
...
mod test {
    ...
    fn test_decode_deposit_invalid_version() {
        ...
        let len: [u8; 8] = U64::from(64).to_be_bytes();
        data[56..64].copy_from_slice(&len);
        // Use version 2 which is invalid
    ...
    fn test_decode_deposit_empty_succeeds() {
        ...
        // Verify the encoded transaction is valid and not empty
        assert!(!tx.is_empty());
        // The exact encoding may vary based on TxDeposit fields, so just verify it's valid
        assert!(tx.len() > 100); // Should be a reasonable length
    }
